...
8. `run: ./xmailman audit list dev@lists.test member.add (expect: alice@lists.test)`
...
11. `run: ./xmailman audit list dev@lists.test role.grant (expect: alice@lists.test)`
...
16. `run: ./xmailman audit list dev@lists.test member.remove (expect: alice@lists.test)`
17. `click "Owner" in row containing "owner@lists.test"`
18. `expect text "cannot remove the last owner" to appear`
19. `run: ./xmailman owner list dev@lists.test (expect: owner@lists.test)`
...
27. `run: ./xmailman audit list mod@lists.test subscription.approve (expect: heldsub2@lists.test)`
...
36. `run: ./xmailman audit list mod@lists.test subscription.reject (expect: heldsub3@lists.test)`
...
40. `run: ./xmailman subscriber export dev@lists.test (expect: email,status,delivery_mode,roles)` (the CLI export shares the same CSV writer as the web export, so this verifies the CSV body the button downloads)
41. `upload ${TMPDIR:-/tmp}/xmailman-e2e-import.csv to "CSV file"` (resolve `$TMPDIR` via the shell; the browser MCP only permits file paths under the OS temp dir)
42. `click "Import"`
...
47. `run: ./xmailman audit list dev@lists.test member.import (expect: imported1@lists.test)`

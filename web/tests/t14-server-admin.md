...
1. `run: ./xmailman list create opsx@lists.test --type discussion --desc "admin fixture list"`
2. `run: ./xmailman subscriber add opsx@lists.test opsadmin@lists.test`
...
8. `run: ./xmailman audit server domain.create (expect: ops.example)`
...
17. `run: ./xmailman audit server list.type (expect: del@lists.test)`
...
25. `run: ./xmailman audit server list.delete (expect: del@lists.test)`
...
31. `run: ./xmailman audit server admin.designate (expect: opsadmin@lists.test)`
32. `click "Revoke" in row containing "opsadmin@lists.test"`
33. `expect text "opsadmin@lists.test" to be absent`
34. `run: ./xmailman audit server admin.revoke (expect: opsadmin@lists.test)`

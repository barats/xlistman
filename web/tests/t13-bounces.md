...
1. `run: ./xmailman list create opsb@lists.test --type discussion --owner owner@lists.test --desc "bounce fixture"`
...
9. `run: ./xmailman audit list opsb@lists.test member.re-enable (expect: dave@lists.test)`
10. `click "Reset count" in row containing "erin@lists.test"`
11. `expect text "Reset bounce count for erin@lists.test." to appear`
12. `run: ./xmailman audit list opsb@lists.test member.reset-bounces (expect: erin@lists.test)`

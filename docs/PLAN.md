...
- Go server serves the SPA with a fallback to `index.html` for client-side routes.
  (building the binary requires the frontend build in `web/build`.)
- The SPA build is generated by `pnpm build`/Docker and gitignored
...
- The full frontend + Go binary build was run end-to-end and exercised in the browser

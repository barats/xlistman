...
# Build the binary.
RUN CGO_ENABLED=0 go build -ldflags "-X github.com/barats/xmailman/cmd.Version=$(git describe --tags --always 2>/dev/null || echo dev)" -o xmailman .
...
COPY --from=builder /build/xmailman /xmailman
COPY config.default.yaml /etc/xmailman/config.yaml

# Point the daemon at the baked-in config; env overrides below win.
ENV XMAILMAN_CONFIG=/etc/xmailman/config.yaml
...
ENV XMAILMAN_DATABASE_PATH=/data/xmailman.db

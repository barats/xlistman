...
# 1. Back up and extend main.cf with the xMailman virtual domain + LMTP transport.
if [ -f /etc/postfix/main.cf.xmailman-validate.bak ]; then
  echo "main.cf backup already exists; not re-applying" >&2
else
  cp /etc/postfix/main.cf /etc/postfix/main.cf.xmailman-validate.bak
  cat >> /etc/postfix/main.cf <<'EOF'

# --- xMailman LMTP loop validation (added by start-postfix.sh) ---
...
if [ -f /etc/aliases.xmailman-validate.bak ]; then
  ...
  cp /etc/aliases /etc/aliases.xmailman-validate.bak
  printf '\n# --- xMailman LMTP loop validation ---\ndeadbeat: no_such_local_user_xyz\n' >> /etc/aliases
fi
